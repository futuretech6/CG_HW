    ...
    while (!file.eof()) {
        ...
        if (operatorCh == "v") {
            ...
            for (objPoint &p : op.points) {
                if (p.id > int(vs.size()) || p.tid > int(vt.size()) ||
                    p.nid > int(vn.size())) {
                    err = true;
                    cout << "Error: " << p.id << "/" << p.tid << "/" << p.nid;
                    cout << "[" << vs.size() << ", " << vt.size() << ", " << vn.size()
                         << "]" << endl;
                    break;
                }
            }
            ...
            switch (op.points.size()) {
                case 3: triangles.push_back(op); break;
                case 4: rects.push_back(op); break;
                default: others.push_back(op);
            }
